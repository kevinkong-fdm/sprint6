...
class DepositWithdrawIntegrationTest {
    ...
    @Test
    void shouldDepositThenWithdraw() throws Exception {
        ...
        when(accountMovementService.withdraw(any(), any(), any(), any())).thenReturn(new MovementResponse(
        ...
        mockMvc.perform(post("/accounts/aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa/deposits")
                ...
                .andExpect(jsonPath("$.balanceAfter").value("125.0000"));

        mockMvc.perform(post("/accounts/aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa/withdrawals")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("""
                                {"amount":10.0}
                                """))
                .andExpect(status().isCreated())
                .andExpect(jsonPath("$.balanceAfter").value("115.0000"));
    }

    @Test
    void shouldRejectInsufficientWithdrawal() throws Exception {
        when(accountAuthorizationService.requireActorId(any())).thenReturn("11111111-1111-1111-1111-111111111111");
        when(accountMovementService.withdraw(any(), any(), any(), any()))
                .thenThrow(new AccountDomainException.InsufficientWithdrawalFundsException());
        ...
        mockMvc.perform(post("/accounts/aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa/withdrawals")
                ...
                .andExpect(jsonPath("$.errorCode").value("TXN-WDR-002"));
    }
}
