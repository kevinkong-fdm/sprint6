...
class RegistrationFlowIntegrationTest {
    ...
    @Test
    void shouldReturnValidationErrorOnInvalidPayload() throws Exception {
        mockMvc.perform(post("/auth/register")
                ...
                .andExpect(jsonPath("$.errorCode").value("AUTH-REG-001"));
    }

    @Test
    void shouldHandleSuccessThenDuplicate() throws Exception {
        when(registrationService.register(any(), any()))
                .thenReturn(new RegisterResponse("user-2", "bob@example.com", Instant.parse("2026-06-25T10:00:00Z")))
